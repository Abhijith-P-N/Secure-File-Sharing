...
echo   seeds the DB, and launches the full stack.
echo ==============================================================
echo.
...
REM ---------------------------------------------------------------
echo.
echo [1/5] Installing backend dependencies (secure-file-backend)...
...
if not exist ".env" (
    ...
    REM Generate random hex keys with Node (works without openssl on PATH)
    ...
    REM Fill the keys (case-insensitive in-place replace of the placeholder lines)
...
REM ---------------------------------------------------------------
REM 3) Frontend: install deps (no env required for dev)
REM ---------------------------------------------------------------
echo.
echo [2/5] Installing frontend dependencies (frontend)...
pushd "%~dp0frontend"
call npm install
...
REM ---------------------------------------------------------------
REM 4) Security module: zero runtime dependencies - nothing to install
REM ---------------------------------------------------------------
echo.
echo [3/5] Security module (security) - zero runtime deps, nothing to install.
...
REM Start embedded PostgreSQL (downloads binaries on first run)
...
REM ---------------------------------------------------------------
REM 6) Seed admin account (waits until backend is healthy)
REM ---------------------------------------------------------------
echo.
echo [5/5] Seeding admin account...
echo   Waiting for backend to become healthy (may take a while on first run)...
set /a attempts=0
:waithealth
        set /a attempts+=1
        if %attempts% gtr 60 (
            echo [WARN] Backend still not responding after 60s. Skipping seeding.
            goto :seed_done
        )
        powershell -NoProfile -Command "try { (Invoke-WebRequest -UseBasicParsing -Uri http://localhost:8000/health/live -TimeoutSec 3).StatusCode } catch { 'down' }" > "%TEMP%\vg_health.tmp" 2>nul
        set /p HEALTH=<"%TEMP%\vg_health.tmp"
        del "%TEMP%\vg_health.tmp" >nul 2>nul
        if "%HEALTH%"=="200" goto :seed_ready
        timeout /t 2 /nobreak >nul
        goto :waithealth
:seed_ready
echo   Backend is up. Seeding admin account...
pushd "%~dp0secure-file-backend"
...
echo   Admin    : admin@secure-share.local / ChangeMe_Admin_2026
