...
if (config.isProduction && (!config.encryptionKey || config.encryptionKey.length < 32)) {
  throw new Error("A FILE_ENCRYPTION_KEY of at least 32 chars is required in production");
}

const key = crypto.createHash("sha256")
  .update(config.encryptionKey || "CHANGE_ME_IN_PRODUCTION")
  .digest();
