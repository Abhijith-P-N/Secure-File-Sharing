...
| POST | /api/auth/logout | Yes | Client-side JWT logout |
...
`GET /api/shares/:token/download` so metadata validation and file transfer stay
separate.
...
For higher assurance, integrate Adhil's file-signature/magic-byte validation
rather than relying only on MIME type.
...
- express-rate-limit: global and stricter authentication limits.
...
```bash
curl http://localhost:3000/api/shares/SHARE_TOKEN
```

For password-protected shares, provide the password during validation:

```bash
curl "http://localhost:3000/api/shares/SHARE_TOKEN?password=SharePass123!"
```

Download a protected share:

```bash
curl -L "http://localhost:3000/api/shares/SHARE_TOKEN/download?password=SharePass123!" -o downloaded.bin
```
